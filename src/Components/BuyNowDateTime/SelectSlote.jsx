    ...
    return (
        <>
            <Box className={classes.cmn_date_time_box}>
                <Box className={`${classes.cmn_date_inner_form_box} ${classes.select_slot_type_wrapper}`}>
                    <FormControl component="fieldset">
                        <RadioGroup aria-label="gender" name="gender1" value={props.value} onChange={props.onChange}>
                            {
                                select_slot.map((sloat, i) => {
                                    return (
                                        <Box className={classes.select_slote_radio_with_price} key={i}>
                                            <FormControlLabel value={sloat.delivery} control={<Radio />} label={sloat.delivery} />
